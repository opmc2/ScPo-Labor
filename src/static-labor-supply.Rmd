...
full_cross_section2 <- function(lb, N, rho, X){ #function to keep track of individuals - same X
  ...
  # we then solve for the choice of hours and consumption
  simdata[, h := pmax(-mu + p$beta0 ,0)/(rho*exp(lw))+1] # starting value
  # for loop for newton method (30 should be enough, it is fast)
  for (i in 1:30) {
      simdata[, h := ff.newt(h,rho*exp(lw),mu-p$beta0,p$eta,p$gamma,p$beta) ]
    }

  # attach consumption, value of working (and participation var for later)
...
full_cross_section <- function(lb, N, rho){ # draws new X in period 2.
  ...
  # attach consumption, value of working (and var for participation later)
  simdata[, c  := rho*exp(lw)*h + mu - p$beta0];
  simdata[, u1 := c^(1+p$eta)/(1+p$eta) - p$beta * h^(1+p$gamma)/(1+p$gamma) ];
  simdata[, u0:=  mu^(1+p$eta)/(1+p$eta)];
  simdata[, p1:=u1>u0]
...
# using dummies
pander(summary(simdata3[,lm(lw ~ log(c) + log(h) + t + q)]))
```
...
# using dummies
pander(summary(simdata3[p1==TRUE,lm(lw ~ log(c) + log(h) + t + q)]))

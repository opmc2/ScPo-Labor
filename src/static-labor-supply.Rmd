...
full_cross_section <- function(lb, N, rho){ # draws new X in period 2.
  #parameters
  p  = list(eta=-1.5,gamma = 0.8,beta=1,beta0=0.1) # define preferences
...
simdata2 <- rbind(full_cross_section(1,10000,1), full_cross_section(1.5,10000,.8))
#simdata2 <- rbind(full_cross_section2(1,10000,1,rnorm(10000)), full_cross_section2(1.5,10000,.8,rnorm(10000)))
pander(summary(simdata2[,lm(lw ~ log(c) + log(h))]))
```
...
# using dummies
pander(summary(simdata3[,lm(lw ~ log(c) + log(h) + factor(q) + t)]))
```
...
```{r control for participation}

simdata2 <- rbind(full_cross_section_IV(1,10000,1), full_cross_section_IV(1.5,10000,.8))
...
attach(simdata3)
select_model <- lm(p1 ~ X + Z + log(c) + log (h))
detach(simdata3)

invisible(simdata3[, preds := predict(select_model)])

w0 <- simdata3[p1 == TRUE & t==0,mean(lw),by=q]
w1 <- simdata3[p1 == TRUE & t==1,mean(lw),by=q]
c0 <- simdata3[p1 == TRUE & t==0,mean(log(c)),by=q]
c1 <- simdata3[p1 == TRUE & t==1,mean(log(c)),by=q]
h0 <- simdata3[p1 == TRUE & t==0,mean(log(h)),by=q]
h1 <- simdata3[p1 == TRUE & t==1,mean(log(h)),by=q]
r0 <- simdata3[p1 == TRUE & t==0,mean(preds), by=q]
r1 <- simdata3[p1 == TRUE & t==1,mean(preds), by=q]
X0 <- simdata3[p1 == TRUE & t==0,mean(X), by=q]
X1 <- simdata3[p1 == TRUE & t==1,mean(X), by=q]
Y0 <- simdata3[p1 == TRUE & t==0,mean(Z), by=q]
Y1 <- simdata3[p1 == TRUE & t==1,mean(Z), by=q]
D <- data.table(q=1:min(length(w0[,q]),length(w1[,q])))
for (iter in 1:10){
  D[q==iter, lw := w1[q==iter,2] - w0[q==iter,2]]
  D[q==iter, lc := c1[q==iter,2] - c0[q==iter,2]]
  D[q==iter, lh := h1[q==iter,2] - h0[q==iter,2]]
  D[q==iter, r := r1[q==iter,2] - r0[q==iter,2]]
  D[q==iter, x := X1[q==iter,2] - X0[q==iter,2]]
  D[q==iter, y := Y1[q==iter,2] - Y0[q==iter,2]]
}



pander(summary(D[,lm(lw ~ lc + lh + r + x + y)]))
```

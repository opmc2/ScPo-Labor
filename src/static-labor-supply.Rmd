...
The combined data (from two cross-sections with different tax rates) produces biased estimates, $\hat{\eta}$ and $\hat{\gamma}$. We have a similar problem to in Questions 1 and 2; we cannot separate out the individual effects through $\beta_i$ and the effects we want to identify i.e. $\eta$ and $\gamma$.
...
```
```{version with same individuals}
simdata3 <- simdata2[,t := rep(c(0,1),each = 10000)]
simdataD <- data.table(i=seq(1:N))
for (iter in 1:N){
  simdataD[i==iter, lc := log(simdata3[i==iter & t==1, c]) - log(simdata3[i==iter & t==0, c])]
  simdataD[i==iter, lw := simdata3[i==iter & t==1, lw] - simdata3[i==iter & t==0, lw]]
  simdataD[i==iter, lh := log(simdata3[i==iter & t==1, h]) - log(simdata3[i==iter & t==0, h])]
}
pander(summary(simdataD[,lm(lw ~ lc + lh)]))
```
...
Following [@BDM1998] we can use an alternative (equivalent) approach to the above group estimation.

```{r}
# alternative (equivalent) approach, using dummies (see BDM (1998))
simdata4 <- simdata3[t==0, lw_at := lw]
simdata4 <- simdata4[t==1, lw_at := log(.8*exp(lw))]

quantX <- quantile(simdata4$X, seq(0,1,.1))
invisible(simdata4[, q:= 0])
for (iter in 1:10){
  simdata4[X > quantX[iter], q := iter]
}
model1 <- simdata4[,lm(lw_at ~ t*q)]
...
full_cross_section_IV <- function(lb, N, rho){ # draws new X in period 2.
  #parameters
  p  = list(eta=-1.5,gamma = 0.8,beta=1,beta0=0.1) # define preferences
  
  #simulating data
  simdata = data.table(i=1:N,X=rnorm(N),Z=rnorm(N)) # add variable Z drawn from rnorm
  simdata[,lw := lb*X + rnorm(N)*0.2];              # add a wage which depends on X and lb
...
To control for participation we want to calculate the probability of working based on the observed variables, and then include this as a regressor in the wage equation. We use the excluded variable that affects wealth but not wages as an instrument.

In the current setup, we could simply include a LPM to calculate the probability of participation and then include the predicted values from this regression in the wage equation:

$$
  \Pr(u_1 > u_0) = \alpha + \beta_1 X + \beta_2 Z + \varepsilon
$$


```{r control for participation, results="hide"}
rho0=1
rho1=.8
simdata2 <- rbind(full_cross_section_IV(1,10000,rho0), full_cross_section_IV(1.5,10000,rho1))
...
quantX <- quantile(simdata3$X, seq(0,1,.25))
for (iter in 1:4){
  simdata3[X >= quantX[iter], q := iter]
}

#because mu changes differentially by z (not used)
quantz <- quantile(simdata3$Z, seq(0,1,.25))
for (iter in 1:4){
  simdata3[Z >= quantz[iter], z := iter]
}

#gives better results than z
quantmu <- quantile(simdata3$mu, seq(0,1,.25))
for (iter in 1:4){
  simdata3[mu >= quantmu[iter], m := iter]
}
...
for (iter in 1:4){
  simdata3[q==iter & t==0, propw := mean(p1)]
  simdata3[q==iter & t==1, propw := mean(p1)]
}
